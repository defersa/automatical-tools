	...
	for( var i = 0; i < eTable.length; i++ ){
		...
		for(var j = 0; j < eTable[i].length; j++){
			if(eTable[i][j] != undefined){
				...
				html += '>';
				if(cell.type == 'thead' || cell.type == 'corn' || cell.type == 'shead')	html += cell.text;
				else if(cell.text !== '')												html += (cell.text + '').replace(/\./g, obj.separator) + obj.units[cell.type];
				html += '</td>';
			}
		}
		html += '</tr>';
...
function toXLSX(options, array){
	var xlsx = {};
	if(!options)									options = {};	
	if(options.oneSheet && array.length == 1)		options.oneSheet = false;
	...
	var sheet_amount = ((options.oneSheet)? (1) : (array.length)) + ((options.TOC)? 1 : 0);
	...
	for(var i = 0; i < array.length; i++){
		...
		if(array[i].object.round != undefined){
			array[i].object.styles = [];

			for(var j = 0; j < array[i].object.round.length; j++){

				if(array[i].object.units[j] == undefined)
					array[i].object.units[j] = '';

				var numFmt_Id = -1;
				for(var k = 0; k < numFmts.length; k++){
					if(numFmts[k].symbol == array[i].object.units[j] && numFmts[k].round == array[i].object.round[j]){
				...
				if(numFmt_Id == -1){
					...
					numFmts[numFmts.length] = {symbol: array[i].object.units[j], round: array[i].object.round[j], id: numFmt_Id};
				}
				var cellXfs_id = -1;
	...
	var colors = (options.background) ? ['FFFFFF','E5FBFF','FFECF1','FFFCEC','F1F1F1'] : ['FFFFFF','FFFFFF','FFFFFF','FFFFFF','FFFFFF'] ;
	...
	for(var l = 0; l < array.length; l++){
		...
		if(array[l].object.table){
			...
			for(var i = 0; i < table.length; i++){
				...
				for(var j = 0; j < table[i].length; j++){
					if(table[i][j] != undefined){
						...
						var cell = table[i][j];
						if(cell.type == 'corn' || cell.type == 'shead' || cell.type == 'thead'){

							if(cell.type == 'corn')												sheet += '<c r="' + (integerToChars(j)) + (row) + '" s="3" t="s"><v>' + ((options.cornerName) ? shareString(replaceText(array[l].annotation)) : '') + '</v></c>';
							else if(cell.type == 'shead' && cell.l)								sheet += '<c r="' + (integerToChars(j)) + (row) + '" s="4" t="s"><v>' + shareString(replaceText(cell.text)) + '</v></c>';
							else if(cell.type == 'shead' && !array[l].object.orientation)		sheet += '<c r="' + (integerToChars(j)) + (row) + '" s="5" t="s"><v>' + shareString(replaceText(cell.text)) + '</v></c>';
							else if(cell.type == 'thead' || cell.type == 'shead')				sheet += '<c r="' + (integerToChars(j)) + (row) + '" s="3" t="s"><v>' + shareString(replaceText(cell.text)) + '</v></c>';
							 							
							if(cell.rows != undefined || cell.cols != undefined){
								var mcol = (cell.cols != undefined) ? cell.cols : 1;
								var mrow = (cell.rows != undefined) ? cell.rows : 1;
								merg[merg.length] = '<mergeCell ref="' + (integerToChars(j)) + (row) + ':' + (integerToChars(j + mcol - 1)) + (row + mrow - 1) + '"/>';
							}
						} else {
							if(typeof cell.text == 'number'){
								if(cell.type == 0 || cell.type == 3 || cell.type == 5 || cell.type == 6)		sheet += '<c r="' + (integerToChars(j)) + (row) + '" s="' + array[l].object.styles[cell.type] + '"><v>' + parseFloat(cell.text) + '</v></c>';
								if(cell.type == 1 || cell.type == 2)											sheet += '<c r="' + (integerToChars(j)) + (row) + '" s="' + array[l].object.styles[cell.type] + '"><v>' + ((array[l].object.units[cell.type])? (parseFloat(cell.text)/100) : parseFloat(cell.text)) + '</v></c>';
							} else																				sheet += '<c r="' + (integerToChars(j)) + (row) + '" s="' + array[l].object.styles[cell.type] + '"/>';
						}
					} else {
			...
			sheet += '<row r="' + row + '" spans="1:1" x14ac:dyDescent="0.25"><c r="A' + row + '" s="3" t="s"><v>' + ((options.cornerName)? shareString(replaceText(array[l].annotation)) : '') + '</v></c></row>';
			row++;
		}
		...
		if(!options.oneSheet){
			...
			sheet = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
						...
						+ '<pageMargins left="0.7" right="0.7" top="0.75" bottom="0.75" header="0.3" footer="0.3"/>' + ( (options.annotation)? '<legacyDrawing r:id="rId1"/>' : '' ) + '</worksheet>';
	...
	if(options.TOC){
		...
		for(var i = 0; i < array.length; i++){
			...
			var sheet_name = (options.sheetNames)? sheet_names[i] : ('Table ' + sheet_names[i]);
		...
		if(options.oneSheet){
			...
			for(var i = 0; i < array.length; i++)		xlsx.xl.sheet[0] += '<hyperlink ref="A' + (i + 1) +'" location="\'' + replaceText( sheet_names[i]) + '\'!A1" display="' + replaceText((options.sheetNames)? sheet_names[i] : ('Table ' + sheet_names[i])) + '"/>';
		}
